    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ background: '#fff', padding: '1rem', borderRadius: 8 }}>
          ...
          <DynamicFieldForm processInstanceId={task.processInstanceId} taskId={task.id} />
        </div>
      </div>

      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
        <button onClick={() => completeMutation.mutate()} style={{ background: '#388e3c', color: '#fff', padding: '0.5rem 1rem', border: 'none', borderRadius: 4, cursor: 'pointer' }}>
          Complete Task
        </button>
      </div>
      ...
      <div style={{ background: '#fff', padding: '1rem', borderRadius: 8 }}>
        ...
        {comments.map((c: any) => (
          <div key={c.id} style={{ borderBottom: '1px solid #eee', padding: '0.5rem 0' }}>
            <strong>{c.userId}</strong> <span style={{ color: '#999', fontSize: '0.85rem' }}>{new Date(c.createdAt).toLocaleString()}</span>
            <p>{c.content}</p>
          </div>
        ))}
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
          <input value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Add comment..." style={{ flex: 1, padding: '0.4rem' }} />
          <button onClick={() => comment && addCommentMutation.mutate(comment)} style={{ padding: '0.4rem 1rem' }}>Add</button>
        </div>
      </div>
    </div>
  );

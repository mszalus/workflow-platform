...
export default function Layout() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh' }}>
      <nav style={{ width: 220, background: '#1a1a2e', color: '#fff', padding: '1rem 0', display: 'flex', flexDirection: 'column' }}>
        <h2 style={{ padding: '0 1rem', marginBottom: '2rem', fontSize: '1.2rem' }}>WFP Admin</h2>
        {navItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            style={{
              ...
              color: location.pathname.startsWith(item.path) ? '#4fc3f7' : '#ccc',
              textDecoration: 'none',
        ...
        <div style={{ marginTop: 'auto', padding: '1rem', borderTop: '1px solid #333' }}>
          ...
          <button onClick={logout} style={{ background: 'none', border: '1px solid #666', color: '#ccc', padding: '0.3rem 0.8rem', cursor: 'pointer', borderRadius: 4 }}>
            Logout
          </button>
        </div>
      </nav>
